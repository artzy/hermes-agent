...
- Cursor = 기본 **agent** (CLI full agent; 파일/셸 가능). Hermes 도구와 겹칠 수 있음
- `HERMES_CURSOR_MODE=ask|agent|plan` (기본 `agent`). `ask`면 Hermes가 도구 소유(읽기 전용)
- 도구 스키마는 프롬프트에 직렬화, 응답의 `<tool_call>` → OpenAI `tool_calls` 파싱
- 인증: `CURSOR_API_KEY` 및/또는 `agent login`
- 런타임: ask는 CLI 고정(`--mode=ask`); agent는 `--mode` 생략(+ SDK 가능 시 SDK)
